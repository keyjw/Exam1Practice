...
def problem2b(rect, n, delta, win):
    ...
    """
    rect.attach_to(win)
    width = rect.get_upper_left_corner()
    height = rect.get_lower_right_corner()

    for k in range(n):
        point1 = rg.Point(width.x - (delta * k), width.y - (delta * k))
        point2 = rg.Point(height.x + (delta * k), height.y + (delta * k))
